...
private const val FOLLOW_CAMERA_DISTANCE_METERS = 15.0
private const val FOLLOW_CAMERA_INTERVAL_MILLIS = 3_000L
...
fun RouteMapView(
    ...
    val cameraFollowState = remember { MapCameraFollowState() }
    val mapView = remember(context) {
        MapView(context).apply {
            OsmdroidDiagnostics.mapViewCreated()
            setTileSource(TileSourceFactory.MAPNIK)
            OsmdroidDiagnostics.tileSourceSet()
            setMultiTouchControls(true)
            controller.setZoom(17.0)
            controller.setCenter(GeoPoint(currentPosition.latitude, currentPosition.longitude))
            cameraFollowState.record(currentPosition)
    ...
    DisposableEffect(lifecycleOwner, mapView) {
        ...
        fun resumeMapView() {
            if (!isResumed && !isDetached) {
                mapView.onResume()
                isResumed = true
                OsmdroidDiagnostics.mapViewResumed()
            }
        }
        ...
        fun pauseMapView() {
            if (isResumed) {
                mapView.onPause()
                isResumed = false
                OsmdroidDiagnostics.mapViewPaused()
            }
        }
        ...
        fun detachMapView() {
            if (!isDetached) {
                ...
                mapView.onDetach()
                isDetached = true
                OsmdroidDiagnostics.mapViewDetached()
            }
        }
            ...
            update = { map ->
                ...
                when {
                    ...
                    followCurrentLocation &&
                        !userMovedCamera &&
                        cameraFollowState.shouldRecenter(currentPosition) -> {
                        map.controller.setCenter(currentGeoPoint)
                        cameraFollowState.record(currentPosition)
                    }
                }
...
private class MapCameraFollowState {
    private var lastPosition: LatLng? = null
    private var lastUpdateElapsedMillis: Long = 0L

    fun record(position: LatLng) {
        lastPosition = position
        lastUpdateElapsedMillis = SystemClock.elapsedRealtime()
    }

    fun shouldRecenter(position: LatLng): Boolean {
        val previousPosition = lastPosition ?: return true
        val elapsedMillis = SystemClock.elapsedRealtime() - lastUpdateElapsedMillis
        return distanceMeters(previousPosition, position) >= FOLLOW_CAMERA_DISTANCE_METERS ||
            elapsedMillis >= FOLLOW_CAMERA_INTERVAL_MILLIS
    }
}
